...
exports.getJoinedGroups = exports.recallMessage = exports.uploadChatFile = exports.removeMember = exports.updateMemberRole = exports.updateGroup = exports.addGroupMember = exports.getChatUsers = exports.markAsRead = exports.deleteConversation = exports.createDirect = exports.createGroup = exports.sendMessage = exports.getMessages = exports.getConversations = void 0;
...
exports.getConversations = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    const conversations = await chatService_1.chatService.getConversations(userId);
    res.status(200).json({
        success: true,
...
exports.getMessages = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    const result = await chatService_1.chatService.getMessages(id, userId, page, limit);
    res.status(200).json({
        success: true,
        ...result
    });
...
exports.sendMessage = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (!text && (!images || images.length === 0) && !file) {
        throw new AppError_1.AppError("Message must have text, images, or file", 400);
    }
    const message = await chatService_1.chatService.sendMessage(id, userId, {
        text,
        images,
        file,
        replyTo
    ...
    await (0, socket_1.emitChatMessage)(id, message, userId.toString());
    res.status(201).json({
        success: true,
        data: message
    });
...
exports.createGroup = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (!name || !memberIds || memberIds.length === 0) {
        throw new AppError_1.AppError("Group name and members are required", 400);
    }
    const conversation = await chatService_1.chatService.createGroup(name, memberIds, userId, avatar, color);
...
exports.createDirect = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const userId = req.user._id || req.user.id;
    const { targetUserId } = req.body;
    if (!targetUserId) {
        throw new AppError_1.AppError("Target user ID is required", 400);
    }
    const result = await chatService_1.chatService.getOrCreateDirect(userId, targetUserId);
...
exports.deleteConversation = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const userId = req.user._id || req.user.id;
    const { id } = req.params;
    await chatService_1.chatService.deleteConversation(id, userId);
    res.status(200).json({
        success: true,
...
exports.markAsRead = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const userId = req.user._id || req.user.id;
    const { id } = req.params;
    await chatService_1.chatService.markAsRead(id, userId);
...
exports.getChatUsers = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const userId = req.user._id || req.user.id;
    const users = await chatService_1.chatService.getChatUsers(userId);
    res.status(200).json({
        success: true,
        count: users.length,
        data: users
...
exports.addGroupMember = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const userId = req.user._id || req.user.id;
    const { id } = req.params;
    const { memberIds } = req.body;
    if (!memberIds || !Array.isArray(memberIds) || memberIds.length === 0) {
        throw new AppError_1.AppError("memberIds array is required", 400);
    }
    const conversation = await chatService_1.chatService.addGroupMember(id, memberIds, userId);
    // Notify all members via Socket.IO
    const io = req.app.get("io");
    if (io) {
...
exports.updateGroup = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    const conversation = await chatService_1.chatService.updateGroup(id, { name, avatar, color }, userId);
    const io = req.app.get("io");
    if (io && conversation) {
...
exports.updateMemberRole = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (!["promote", "demote"].includes(action)) {
        throw new AppError_1.AppError("Action must be 'promote' or 'demote'", 400);
    }
    const conversation = await chatService_1.chatService.updateMemberRole(id, memberId, action, userId);
    const io = req.app.get("io");
    if (io && conversation) {
        conversation.participants.forEach((p) => {
            const pid = p._id || p.id;
...
exports.removeMember = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const userId = req.user._id || req.user.id;
    const { id, memberId } = req.params;
    const conversation = await chatService_1.chatService.removeMember(id, memberId, userId);
    const io = req.app.get("io");
    if (io && conversation) {
...
exports.uploadChatFile = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const file = req.file;
    if (!file) {
    ...
    if (file.size > 20 * 1024 * 1024) {
        throw new AppError_1.AppError("File size cannot exceed 20MB", 400);
    }
...
exports.getJoinedGroups = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const userId = req.user._id || req.user.id;
    const groups = await chatService_1.chatService.getJoinedGroups(userId.toString());
    res.status(200).json({
        success: true,
        data: groups
    });
