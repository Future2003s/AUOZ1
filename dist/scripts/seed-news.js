...
async function seedNews() {
    try {
        logger_1.logger.info("🌱 Starting news seeding...");
        // Connect to MongoDB
        await (0, database_1.connectDatabase)();
        // Check existing news
        const existingCount = await News_1.News.countDocuments();
        if (existingCount > 0) {
            logger_1.logger.info(`Found ${existingCount} existing news articles. Will add new ones without duplicates.`);
        }
        ...
        for (const article of newsToInsert) {
            const existing = await News_1.News.findOne({ slug: article.slug });
            if (existing) {
                logger_1.logger.info(`⏭️  Skipping duplicate: ${article.slug}`);
                skipped++;
                continue;
            }
            await News_1.News.create(article);
            inserted++;
            logger_1.logger.info(`✨ Created: [${article.locale.toUpperCase()}] ${article.title}`);
        }
        logger_1.logger.info(`\n✅ News seeding completed!`);
        logger_1.logger.info(`📊 Results: ${inserted} created, ${skipped} skipped`);
    }
    catch (error) {
        logger_1.logger.error("❌ Error seeding news:", error);
        process.exit(1);
    }
    finally {
