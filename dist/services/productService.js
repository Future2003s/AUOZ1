...
class ProductService {
    ...
    static async getProducts(filters = {}, query = {}) {
        try {
            ...
            if (filters.search && filters.search.trim()) {
                ...
                const mainQuery = Product_1.Product.find(filterQuery).lean();
                const mainResults = await mainQuery.exec();
                // Then search in category and brand collections
                const categoryMatches = await Category_1.Category.find({ name: searchRegex }).select('_id').lean();
                const brandMatches = await Brand_1.Brand.find({ name: searchRegex }).select('_id').lean();
                ...
                if (categoryIds.length > 0 || brandIds.length > 0) {
                    const additionalResults = await Product_1.Product.find(additionalQuery).lean().exec();
                    const allProductIds = new Set([
                        ...mainResults.map((p) => p._id.toString()),
                        ...additionalResults.map((p) => p._id.toString())
                    ]);
                    if (allProductIds.size > 0) {
                        filterQuery._id = { $in: Array.from(allProductIds).map(id => new mongoose_1.Types.ObjectId(id)) };
                    }
                }
            }
    ...
    static async getProductById(productId) {
        try {
            ...
            if (!objectIdPattern.test(productId)) {
                logger_1.logger.warn(`getProductById called with non-ObjectId: ${productId}`);
                throw new AppError_1.AppError(`Invalid product ID format: "${productId}". Expected 24-character hexadecimal ObjectId. Use getProductBySlug() for slug lookup.`, 400);
            }
            // Try cache first
    ...
    static async getProductBySlug(slug) {
        try {
            ...
            if (objectIdPattern.test(slug)) {
                logger_1.logger.warn(`getProductBySlug called with ObjectId: ${slug}. Use getProductById() instead.`);
                // Still try to find by slug first, but also suggest using getProductById
            }
            const cacheKey = `product:slug:${slug}`;
            const cached = await this.cache.get(cacheKey);
            if (cached) {
                logger_1.logger.debug(`Cache HIT for slug: ${slug}`);
                return cached;
            }
            logger_1.logger.debug(`Cache MISS for slug: ${slug}, querying database...`);
            ...
            if (!product) {
                throw new AppError_1.AppError(`Product with slug "${slug}" not found`, 404);
            }
            await this.cache.set(cacheKey, product, redis_1.CACHE_TTL.MEDIUM);
            return product;
        }
     ...
     */
    static async getNuocCotVai100Product() {
        try {
            ...
            for (const pattern of searchPatterns) {
                try {
                    product = await Product_1.Product.findOne({
                        ...
                        .lean();
                    if (product) {
                        logger_1.logger.info(`Found Nuoc Cot Vai 100 product with pattern: ${JSON.stringify(pattern)}`);
                        break;
                    }
                }
                catch (patternError) {
                    // Skip invalid patterns and continue
                    logger_1.logger.warn(`Pattern search failed: ${JSON.stringify(pattern)}`, patternError);
                    continue;
                }
            }
            // If still not found, try a broader search for any product with "vải" in name
            if (!product) {
                logger_1.logger.warn("Nuoc Cot Vai 100 product not found with specific patterns, trying broader search...");
                product = await Product_1.Product.findOne({
                    name: { $regex: /vải|vai/i },
                    status: "active",
                    isVisible: true
            ...
            if (!product) {
                logger_1.logger.error("No product found matching 'Nước Cốt Vải 100% Thanh Hà' or any similar product");
                throw new AppError_1.AppError("Nước Cốt Vải 100% Thanh Hà product not found. Please create this product in the database first.", 404);
            }
            // Cache the result for longer since this is a specific product
            await this.cache.set(cacheKey, product, redis_1.CACHE_TTL.LONG);
            return product;
        }
        catch (error) {
            logger_1.logger.error("Get Nuoc Cot Vai 100 product error:", error);
            throw error;
        }
    }
    /**
    ...
    static async updateProduct(productId, updateData, userId) {
        try {
            ...
            const isFeaturedUpdated = sanitizedUpdate.hasOwnProperty("isFeatured") && sanitizedUpdate.isFeatured !== wasFeatured;
            // Update product
            ...
            if (isFeaturedUpdated ||
                ...
                await this.invalidateProductCache("featured:*");
                logger_1.logger.info(`Featured products cache invalidated due to isFeatured/status/isVisible change for product ${productId}`);
            }
    ...
    static buildProductFilterQuery(filters) {
        ...
        if (filters.inStock !== undefined) {
            if (filters.inStock) {
                orConditions.push({ trackQuantity: false }, { quantity: { $gt: 0 } }, { allowBackorder: true });
            }
            else {
        ...
        if (filters.search) {
            ...
            if (searchTerm) {
                ...
                searchConditions.push({ 'seo.title': searchRegex }, { 'seo.description': searchRegex }, { 'seo.keywords': { $regex: escapedTerm, $options: 'i' } } // Search in keywords array
                );
    ...
    static async invalidateProductCache(pattern) {
        try {
            ...
            logger_1.logger.info(`Product cache invalidated${pattern ? ` with pattern: ${pattern}` : ""}`);
        }
        catch (error) {
    ...
    static async getFeaturedProducts(limit = 10) {
        try {
            ...
            const cached = await this.cache.get(cacheKey);
            if (cached) {
                logger_1.logger.info(`Featured products cache HIT: Found ${cached.length} products`);
                return cached;
            }
            logger_1.logger.info("Featured products cache MISS: Fetching from database");
            ...
            // Debug: Check counts
            const totalFeatured = await Product_1.Product.countDocuments({ isFeatured: true });
            const activeFeatured = await Product_1.Product.countDocuments({ isFeatured: true, status: "active" });
            const visibleActiveFeatured = await Product_1.Product.countDocuments({
                isFeatured: true,
                status: "active",
                isVisible: true,
                ...excludeHoneyQuery
            });
            logger_1.logger.info(`Featured products debug: total with isFeatured=true: ${totalFeatured}, active: ${activeFeatured}, visible+active: ${visibleActiveFeatured}`);
            ...
            const products = await Product_1.Product.find({
                ...
                .limit(limit)
                .lean();
            logger_1.logger.info(`Found ${products.length} featured products from database`);
            if (products.length === 0) {
                logger_1.logger.warn("⚠️ No featured products found. Check if products have isFeatured=true, status='active', and isVisible=true");
            }
